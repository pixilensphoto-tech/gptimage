...
export default function Gpt2Page() {
  ...
  async function pollJob(jobId: string) {
    ...
    for (let attempt = 0; attempt < 180; attempt += 1) {
      const response = await fetch(`/gpt2/api/generate/${jobId}`, { cache: "no-store" });
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/gpt2/api/generate", { method: "POST", body: formData });
        ...
        <header className="flex items-center justify-between gap-4 rounded-[2rem] border border-white/10 bg-white/[0.06] px-6 py-5 shadow-2xl shadow-black/30 backdrop-blur md:px-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="animated-logo-text pb-2 text-3xl font-black leading-[1.18] tracking-tight md:text-5xl">GPT Image Creator 2</h1>
            </div>
          </div>
